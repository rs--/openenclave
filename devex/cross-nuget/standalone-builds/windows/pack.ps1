...
$LibCopyRules = @(
    New-Object PSObject -Property @{Source="\source\Build\Default\Debug";Destination="\source\Pack\lib\native\win\sgx\default\debug"}
    New-Object PSObject -Property @{Source="\source\Build\Default\Release";Destination="\source\Pack\lib\native\win\sgx\default\release"}
)
...
Function Copy-Tools([String]$SgxPlatform)
{
    ...
    Pop-Location
    Copy-Item -Path "$($Bin.FullName)\*" -Destination \source\Pack\tools\win\default -Recurse -Force
}
...
Function Copy-DebugTools()
{
    ...
    $Locations = @(
        New-Object PSObject -Property @{folder="\source\Build\Default\Debug\host\CMakeFiles\oehost.dir";file="oehost.pdb";destination="\source\Pack\lib\native\win\sgx\default\debug\host\msvc-14.16.27023"}
        New-Object PSObject -Property @{folder="\source\Build\Default\Release\debugger\debugrt\host";file="oedebugrt.*";destination="\source\Pack\tools\win\default"}
    )
...
New-Item -ItemType Directory -Path \source\Pack\tools\win\default | Out-Null
